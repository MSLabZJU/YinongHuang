...
m = size(X,1);
Classifier = {'LR','SVM','SVM_G','LDA'};
% for j = 2:2
for j = 3:size(Classifier,2)
    class = Classifier{j};
for t = 1:9
    Accuracy = [];
...
p = predict(SVMModel, Xtest);
elseif strcmp(class,'LDA')
fprintf('\nTraining Discriminat Analysis ...\n')
MdlLinear = fitcdiscr(Xtrain,ytrain,'DiscrimType','linear');
p = predict(MdlLinear, Xtest);
end
...
save(strcat('Accuracy_',class,'_',num2str(t),'.mat'),'Accuracy');
end
end